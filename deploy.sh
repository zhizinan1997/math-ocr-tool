...
# 设置默认值
IMAGE_NAME="math-latex-converter:latest"
...
# 检查镜像是否存在，如果不存在则提示用户
if ! docker images | grep -q "math-latex-converter"; then
    echo ""
    echo "⚠️ 本地没有找到镜像，请选择："
    echo "1. 使用预构建镜像 (推荐)"
    echo "2. 从源码构建"
    ...
    if [ "$BUILD_CHOICE" = "2" ]; then
        ...
        docker build -t math-latex-converter:latest .
        if [ $? -ne 0 ]; then
            echo "❌ 镜像构建失败"
            exit 1
        fi
        ...
        echo "📥 拉取预构建镜像..."
        docker pull zhizinan/math-latex-converter:latest
        if [ $? -ne 0 ]; then
            echo "❌ 镜像拉取失败，请检查网络连接或使用源码构建"
            exit 1
        fi
        IMAGE_NAME="zhizinan/math-latex-converter:latest"
    fi
fi
